...
const calc = (x: number, y: number, rect: DOMRect, onlyImg: boolean): [number, number, number] => [
    ...
    >
      ...
      >
        <Link href={href} aria-label={`Link to ${title}`}>
          ...
          >
            {imgSrc && (
              <Image alt={title as string} src={imgSrc} className="object-cover object-center" />
            )}
            {onlyImg && <>{children}</>}
            {!onlyImg && (
              <div className="p-4">
